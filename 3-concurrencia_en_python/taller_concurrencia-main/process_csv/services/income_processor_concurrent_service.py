...
class IncomeProcessorConcurrentService(IncomeProcessorService):
    def average_per_region(self, incomes: List[Income]) -> Dict[str, DataIncome]:
        ...
        with ProcessPoolExecutor() as executor:
            futures = [
                executor.submit(self._calculate_average, region=region, amounts=amounts)
                for region, amounts in region_incomes.items()
            ]

            region_amount = {}
            for future in futures:
                result = future.result()
                region_amount[result[0]] = result
    ...
    def _calculate_average(self, region: str, amounts: List[float]) -> Tuple[str, DataIncome]:
        ...
        return region, DataIncome(
            average=sum(amounts) / len(amounts), min=min(amounts), max=max(amounts)
        )
